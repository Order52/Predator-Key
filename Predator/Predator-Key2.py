...
def save_cache(device_path, key_code, key_type):
        ...
        for device in devices:
            try:
                ...
                for event in device.read():
                    is_standard_key = (event.type == evdev.ecodes.EV_KEY and event.value == 1 and event.code not in EXCLUDED_KEYS)
                    is_abs_misc_key = (event.type == evdev.ecodes.EV_ABS and event.code == evdev.ecodes.ABS_MISC and event.value == 1)

                    if is_standard_key or is_abs_misc_key:
                        key_type = event.type
                        key_type_name = "EV_KEY" if is_standard_key else "EV_ABS"
                        key_name = "UNKNOWN"
                        ...
                        print(f"  Key Code: {event.code} ({key_type_name})")
                        print(f"  Key Name: {key_name}")

                        save_cache(device.path, event.code, key_type)
                        detected = True
                        print("\nDetection complete! Continuing to monitor...\n")
                        return device.path, event.code, key_type
...
def find_predator_device():
    ...
    if cache:
        ...
        key_type = cache.get("key_type") # <-- Added this
        if device_path and os.path.exists(device_path) and key_code is not None and key_type is not None:
            print(f"✓ Using cached device: {device_path} (key code: {key_code}, type: {key_type})")
            print("  (To re-detect, delete the cache file: /tmp/predator_key.json)\n")
            return device_path, key_code, key_type
...
def handle_predator_key():
    device_path, key_code, key_type = find_predator_device()
    
    if not device_path or key_code is None or key_type is None:
    ...
    try:
        ...
        for event in device.read_loop():
            if event.type == key_type and event.code == key_code:
...
def check_singleton():
    ...
    try:
        ...
        print("  pkill -f Predator-Key.py") # NOTE: Assumes script name is Predator-Key.py
        sys.exit(1)
